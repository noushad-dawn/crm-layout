...
const AssignTable = () => {
  ...
  const fetchOrders = async (searchTerm = "") => {
    try {
      let url = `api/orders`;
      if (searchTerm.trim() !== "") {
        url = `api/orders/search?query=${encodeURIComponent(searchTerm)}`;
      }
  ...
  const fetchRoutes = async () => {
    try {
      const response = await api.get(`api/route-location/routes`);
  ...
  const assignDriver = async (driver) => {
    try {
      const response = await api.post(
        `api/orders/assign/${selectedOrder.orderId}`,
